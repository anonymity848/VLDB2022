...
void UH_Random(point_set* pset, point_t* e, long mem_baseline, int type)
{
	...
	while (C_idx.size() > 1)
	{
        ...
        else
        {
            ...
            if(dist1 > dist2)
            {
                if (point1->attr[d - 1] < point2->attr[d - 1])
                {
        ...
        printMiddleResult(out_cp, t1, preTime, Qcount, (C_idx.size()/pointSize) * 100,
                          (1 - C_idx.size()/LastCsize) * 100, mem_baseline, type);
        LastCsize = C_idx.size();
    }

	pset->points[C_idx[0]]->printResult(out_cp, "UH-Random", Qcount, t1, preTime, mem_baseline, pointSize, type);
    out_cp.close();
}
