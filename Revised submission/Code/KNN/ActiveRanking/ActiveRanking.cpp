...
int ActiveRanking(point_set* pset, point_t* e, long mem_baseline, int type)
{
    ...
    current_use[0]->printResult(out_cp, "ActiveRanking", Qcount, t1, preTime, mem_baseline, M, type);
    out_cp.close();
}
...
int ActiveRankingTopk(point_set* pset, point_t* e, int k, long mem_baseline, int type)
{
    ...
    for(int i = 0; i < k; ++i)
        resultSet->points.push_back(current_use[i]);
    resultSet->printResult(out_cp, "ActiveRankingTopk", Qcount, t1, preTime, mem_baseline, M, type);
    out_cp.close();
}
