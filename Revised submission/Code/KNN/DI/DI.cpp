 ...
 */
void DI(point_set *pset, point_t *e, long mem_baseline, int type)
{
    ...
    while (candPoint.size() > 1)
    {
        ...
        printMiddleResult(out_cp, t1, preTime, Qcount, (candPoint.size()/pointSize) * 100,
                          (1 - candPoint.size()/LastCsize) * 100, mem_baseline, type);
        LastCsize = candPoint.size();
    }
    candPoint[0]->printResult(out_cp, "DI", Qcount, t1, preTime, mem_baseline, pointSize, type);
    out_cp.close();
}