 ...
 */
point_set *point_set::findNearestK(point_t *e, int k)
{
    point_set *topSet = new point_set();
    std::vector<double> valueSet;
    //set the initial k points
    topSet->points.push_back(points[0]);
    valueSet.push_back(points[0]->distance(e));
    for (int i = 1; i < k; i++)
    {
        double value = points[i]->distance(e);
        int j = 0;
                          ...
                          double Pcize, int type)
{
    ...
    std::cout << "--------------------------------------------------------------------------------------\n";
    printf("|%20s |%15d |%15lf |%15lf |%10d |\n", name, Qcount, preTime, time_cost - preTime, points[0]->id);// get_mem_usage() - mem_baseline);
