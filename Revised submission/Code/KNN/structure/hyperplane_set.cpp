 ...
 * @param t       label: consider all dimensions
 */
hyperplane_set::hyperplane_set(point_set* p_set, int t)
{
 ...
 * @param The point set
 */
void hyperplane_set::nearestSkyband(point_set *pset, int k)
{
    int i = 0;
    while (i < pset->points.size())
    {
...
point_set* hyperplane_set::findPossibleNearestK(point_set *pset, int k)
{
    int M = pset->points.size();
    int dim = pset->points[0]->d, dimo = pset->points[0]->d_order, dimu = pset->points[0]->d_unorder;
    point_t *p = new point_t(dimo, dimu);
    for(int i = 0; i < dimo; ++i)
        p->attr[i] = expDim->attr[i];
    point_t *pp = average_point();
    ...
    //p->print();
    point_set *topSet1 = pset->findNearestK(p, k), *topSet2 = new point_set();
    int relation;
    ...
    for(int i = 0; i < topSet1->points.size(); ++i)
    {
        ...
        else
        {
            ...
            {
                hyperplane *h = new hyperplane(topSet1->points[i],pset->points[topSet1->points[i]->notRdominateID[j]], expDim);
                relation = check_relation(h);
                delete h;
                if (relation == 1)
                {
            ...
            if(topSet1->points[i]->notRdominateID.size() < k)
            {
                ...
                for (int j = topSet1->points[i]->currentScanID; j < M; ++j)
                {
                    ...
                    {
                        hyperplane *h = new hyperplane(topSet1->points[i], pset->points[j], expDim);
                        relation = check_relation(h);
                        delete h;
                        if (relation == 1)
                        {
...
{
    //char filename[MAX_FILENAME_LENG];
    int dim = pset->points[0]->d, M = pset->points.size();
    ofstream wPtr;
    wPtr.open(filename, std::ios::out);
    wPtr.setf(ios::fixed, ios::floatfield);  // set as fixed model
    wPtr.precision(6);  // set precision to 6
...
bool hyperplane_set::findRegion(point_set *origSet, point_set *pset, point_set *cPoint, std::vector<hyperplane_set*> &PartitionSet)
{
    std::vector<hyperplane_set*> Set;
    std::vector<point_set*> PtSet;
    char file1[MAX_FILENAME_LENG];
    sprintf(file1, "../output/candPoint.txt");
    char file2[MAX_FILENAME_LENG];
    sprintf(file2, "../output/partition.txt");
    ...
    int M = pset->points.size(), HM, d = pset->points[0]->d, para;
    int site1, site2;
    for(int i = 0; i < M; ++i)
        PtSet.push_back(new point_set());
    ...
    if(pset->points[0]->d_unorder > 1)
    {
        for (int t = 0; t < 2; ++t)
        {
            ...
            {
                //std::cout << "hyper: " << i <<"\n";
                fscanf(rPtr, "%i%i%i", &para, &site1, &site2);
                double offset, *coord1 = new double[d + 1];
                for (int j = 0; j < d + 1; ++j)
                    fscanf(rPtr, "%lf", &coord1[j]);
                PtSet[site1]->points.push_back(pset->points[site2]);
                PtSet[site2]->points.push_back(pset->points[site1]);
            }
        }
        fclose(rPtr);
        ...
        for (int i = 0; i < M; ++i)
        {
            ...
            for (int j = 0; j < hyperplanes.size(); ++j)
                hSet->hyperplanes.push_back(new hyperplane(hyperplanes[j]));
            for (int j = 0; j < PtSet[i]->points.size(); ++j)
            {
                hyperplane *h = new hyperplane(PtSet[i]->points[j], pset->points[i], expDim);
                hSet->hyperplanes.push_back(h);
            }
            if (hSet->set_ext_pts())
            {
                PartitionSet.push_back(hSet);
                hSet->expDim = hSet->average_point();
                pset->points[i]->internal = hSet->find_feasible();
                cPoint->points.push_back(pset->points[i]);
                pset->points[i]->boundaryHyperID = PartitionSet.size() - 1;
            }
        }
    ...
    {
        for(int i = 0; i < M; ++i)
            Set.push_back(new hyperplane_set(origSet, 1));
        for (int t = 0; t < 2; ++t)
        {
            fscanf(rPtr, "%i", &HM);
            for (int i = 0; i < HM; ++i)
            {
                //std::cout << "hyper: " << i <<"\n";
                fscanf(rPtr, "%i%i%i", &para, &site1, &site2);
                double offset, *coord1 = new double[d], *coord2 = new double[d];
                for (int j = 0; j < d; ++j)
                {
                    fscanf(rPtr, "%lf", &coord1[j]);
                    coord2[j] = -coord1[j];
                }
                fscanf(rPtr, "%lf", &offset);
                hyperplane *h1 = new hyperplane(d, coord1, offset);
                hyperplane *h2 = new hyperplane(d, coord2, -offset);
                if(h1->check_positive(pset->points[site1]))
                {
                    Set[site1]->hyperplanes.push_back(h2);
                    Set[site2]->hyperplanes.push_back(h1);
                }
                else
                {

                    Set[site1]->hyperplanes.push_back(h1);
                    Set[site2]->hyperplanes.push_back(h2);
                }
            }
        }
        fclose(rPtr);
        ...
        int dimo = expDim->d_order, dimu = expDim->d_unorder, count = 0;
        for(int i = 0; i < M; ++i)
        {
            bool setting = Set[i]->set_ext_pts();
            //Set[i]->print();
            ...
            {
                for (int j = 0; j < Set[i]->ext_pts.size(); ++j)
                {
                    bool isture = true;
                    for (int k = 0; k < dimo; ++k)
                    {
                        if (Set[i]->ext_pts[j]->attr[k] - expDim->attr[k] > EQN3 ||
                            Set[i]->ext_pts[j]->attr[k] - expDim->attr[k] < -EQN3)
                        {
                            isture = false;
                            break;
                        }
                    ...
                    if (!isture)
                    {
                        Set[i]->ext_pts.erase(Set[i]->ext_pts.begin() + j);
                        j--;
                    }

                }
                if (Set[i]->ext_pts.size() != 0)
                {
                    //Set[Set.size() - 1]->expDim = pset->points[i];
                    hyperplane_set *hSet = new hyperplane_set();
                    for(int j = 0; j < Set[i]->ext_pts.size(); ++j)
                    {
                        point_t *p = new point_t(dimu);
                        for(int k = 0; k < dimu; ++k)
                            p->attr[k] = Set[i]->ext_pts[j]->attr[dimo + k];
                        hSet->ext_pts.push_back(p);
                    }
                    if(hSet->ext_pts[0]->attr[0] < hSet->ext_pts[1]->attr[0])
                    {
                        point_t *p = hSet->ext_pts[1];
                        hSet->ext_pts.pop_back();
                        hSet->ext_pts.insert(hSet->ext_pts.begin(), p);
                    }
                    pset->points[i]->internal = hSet->average_point();
                    cPoint->points.push_back(pset->points[i]);
                    PartitionSet.push_back(hSet);
                    //hSet->print();
                    for(int t= 0; t < hSet->ext_pts.size(); ++t)
                        pset->points[i]->ext.push_back(hSet->ext_pts[t]);
                }
            }
        }
...
/**
 * @brief Update the expDim of R
 */
void hyperplane_set::update_expDim()
{
    int dimo = expDim->d_order, dimu = expDim->d_unorder;
    point_t *p =average_point();
    for(int j = dimo; j < dimo + dimu; ++j)
        expDim->attr[j] = p->attr[j-dimo];
}



point_t *hyperplane_set::maxpt(point_t *p1, point_t *p2)
{
    int dim = hyperplanes[0]->dim;
    double max = INF;
    for (int i = 0; i < hyperplanes.size(); ++i)
    {
        double cof = 0, off = -hyperplanes[i]->offset;
        for(int j = 0; j < dim; ++j)
        {
            cof += (p1->attr[j] - p2->attr[j]) * hyperplanes[i]->norm[j];
            off -= p1->attr[j] * hyperplanes[i]->norm[j];
        }
        if(cof > 0 && (off/cof) < max)
            max = off/cof;
    }
    point_t* p = new point_t(dim);
    for(int j = 0; j < dim; ++j)
        p->attr[j] = (p1->attr[j] - p2->attr[j]) * max + p1->attr[j];
    return p;
}

point_t *hyperplane_set::minpt(point_t *p1, point_t *p2)
{
    ...
    for (int i = 0; i < hyperplanes.size(); ++i)
    {
        ...
        if(cof < 0 && (off/cof) > min)
            min = off/cof;
    }
    point_t* p = new point_t(dim);
    for(int j = 0; j < dim; ++j)
        p->attr[j] = (p1->attr[j] - p2->attr[j]) * min + p1->attr[j];
    return p;
}
